  ...
  const handlePointerMove = useCallback((e: PointerEvent) => {
    if (pointerInteracting.current !== null) {
      ...
      if (lastPointer.current) {
        ...
        const maxVel = 0.15
        velocity.current = {
          phi: Math.max(-maxVel, Math.min(maxVel, ((e.clientX - lastPointer.current.x) / dt) * 0.3)),
          theta: Math.max(-maxVel, Math.min(maxVel, ((e.clientY - lastPointer.current.y) / dt) * 0.08)),
        }
      }
