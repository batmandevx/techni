...
export default function DashboardPage() {
  ...
  const stats = {
    ...
    ordersChange: hasRealData ? dashboardData.ordersChange : 8.5,
    revenue: hasRealData ? dashboardData.revenue : 68400,
    revenueChange: hasRealData ? dashboardData.revenueChange : 12.3,
    customers: hasRealData ? dashboardData.customers : 89,
    customersChange: hasRealData ? dashboardData.customersChange : 5.2,
    products: hasRealData ? dashboardData.products : 156,
    productsChange: hasRealData ? dashboardData.productsChange : 3.1,
    forecastAccuracy: hasRealData ? dashboardData.forecastAccuracy : 87.5,
    accuracyChange: hasRealData ? dashboardData.accuracyChange : 2.8,
    inventoryValue: hasRealData ? dashboardData.inventoryValue : 68400000,
    inventoryChange: hasRealData ? dashboardData.inventoryChange : -4.2,
  };

  const revenueData = hasRealData && dashboardData.revenueTrend.length
    ? dashboardData.revenueTrend
    : defaultRevenueData;

  const categoryData = hasRealData && dashboardData.categoryDistribution.length
    ? dashboardData.categoryDistribution
    : defaultCategoryData;

  const abcData = hasRealData && dashboardData.abcClassification.length
    ? dashboardData.abcClassification
    : defaultAbcData;

  const performanceData = hasRealData && dashboardData.performanceMetrics.length
    ? dashboardData.performanceMetrics
    : defaultPerformanceData;

  const radarData = hasRealData && dashboardData.radarMetrics.length
    ? dashboardData.radarMetrics
    : defaultRadarData;
  const funnelData = hasRealData && dashboardData.funnelData.length
    ? dashboardData.funnelData
    : defaultFunnelData;
  const heatmapData = hasRealData && dashboardData.heatmapData.length
    ? dashboardData.heatmapData
    : defaultHeatmapData;
  const topProducts = hasRealData && dashboardData.topProducts.length
    ? dashboardData.topProducts
    : defaultTopProducts;
  const regionalData = hasRealData && dashboardData.regionalData.length
