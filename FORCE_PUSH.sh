...
git config user.email "dev@tenchi.com" 2>/dev/null || true
git config user.name "Tenchi Dev" 2>/dev/null || true
...
REQUIREMENTS:
npm install @clerk/nextjs" 2>/dev/null || echo "Nothing to commit"
